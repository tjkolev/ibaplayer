...
bool MsgProcessor::init(IBAPlayer& iba)
{
    ...
    //TODO m_lib = &iba.getLibMngr().getBrowser();
...
void MsgProcessor::startPlaying()
{
    ...
    m_alsa->clear();
    //TODO m_alsa->add(m_lib->getDefaultPlaylist());
    m_alsa->setLoop(true);
...
void MsgProcessor::handlePlayButton(int button)
{
    switch(button)
    {
        ...
        case MID_BUTN_CD1: // Brwose: Art
            Log("Play: Browse artist.", IBALogger::LOGS_DEBUG);
            toBrowseMode();
            break;
        case MID_BUTN_CD2: // Browse: Alb
            Log("Play: Browse album.", IBALogger::LOGS_DEBUG);
            toBrowseMode();
            break;
        case MID_BUTN_CD3: // Browse: Trk
            Log("Play: Browse track.", IBALogger::LOGS_DEBUG);
            toBrowseMode();
            break;
        case MID_BUTN_CD4: // Browse: Gen
            Log("Play: Browse genre.", IBALogger::LOGS_DEBUG);
            toBrowseMode();
            break;

        case MID_RANDOM_ON: // Random
            Log("Play: Random on.", IBALogger::LOGS_DEBUG);
            m_randomOn = true;
            //m_alsa->setShuffle(m_randomOn);
        ...
        case MID_RANDOM_OFF: // Random
            Log("Play: Random off.", IBALogger::LOGS_DEBUG);
            m_randomOn = false;
            //m_alsa->setShuffle(m_randomOn);
            setMIDLabels(MODE_PLAY); // refresh labels
            break;
...
void MsgProcessor::handleBrowseButton(int button)
{
    ...
    switch(button)
    {
        ...
        case MID_BUTN_CD3: // DrillIn
            Log("Browse: LevelDn.", IBALogger::LOGS_DEBUG);
            //TODO txt = m_lib->lvlDn().c_str();
            break;
        case MID_BUTN_CD4: // Back
            Log("Browse: LevelUp.", IBALogger::LOGS_DEBUG);
            //TODO txt = m_lib->lvlUp().c_str();
            break;
        case MID_BUTN_CD5: // play
            Log("Browse: Play.", IBALogger::LOGS_DEBUG);
            m_alsa->clear();
            //TODO m_alsa->add(m_lib->Select());
            m_alsa->setLoop(true);
            //m_alsa->setShuffle(m_randomOn);
        ...
        case MID_RANDOM_OFF:
            Log("Browse: Esc.", IBALogger::LOGS_DEBUG);
...
void MsgProcessor::toBrowseMode()
{
    ...
    //TODO txt = m_lib->setStartLevel(lvl).c_str();
    time(&m_browseTime);
    if(NULL == txt)
    {
        Log("No text from setStartLevel().", IBALogger::LOGS_DEBUG);
        return;
    }
